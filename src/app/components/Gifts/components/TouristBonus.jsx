...
export default function TouristBonus() {
    ...
    const getBinary = async () => {
        const cachedBinary = getCookie('TouristBonus');
        if (cachedBinary) {
        ...
        try {
            const response = await axios.get(`${API_URL}/api/v1/tour/bonuses?page=1&page_size=20`, {
                headers: {
            ...
            setCookie('TouristBonus', JSON.stringify(response.data.participants), 7); // Кэшируем на 7 дней
        } catch (error) {
            console.error(error);
        }
    };
                ...
                <tbody>
                    {binaty.map((item, index) => (
                        <tr key={index}>
                            ...
                            <td>{formatDate2(item.active_time)}</td>
